...
namespace XIVMarketBoard_Api.Controller
{
    ...
    public class RecipeController : IRecipeController
    {
        ...
        public async Task<IEnumerable<Recipe>> GetAllRecipesList() => await _xivContext.Recipes.Include(r => r.Ingredients).ThenInclude(p => p.Item).Include(r => r.Item).Include(r => r.job).Where(r => r.Id != null).ToListAsync();
